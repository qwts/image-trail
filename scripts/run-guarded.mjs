...
// Repository-owned process-tree guard for test commands (#670–#673).
//
// Wraps a command in its own process group and enforces, across the ENTIRE
// descendant tree (node workers, browsers, helpers):
//   - an aggregate RSS ceiling (the 2026-07-18 incident grew ~2 GB/s and took
//     down the machine; V8's --max-old-space-size alone cannot catch that),
...
export function resolveLimits(options, env) {
  ...
  return {
    rssMb: pick('IMAGE_TRAIL_GUARD_RSS_MB', options.rssMb, DEFAULTS.rssMb),
...
function main() {
  ...
  const child = spawn(command[0], command.slice(1), {
    ...
    env: { ...process.env, IMAGE_TRAIL_GUARDED: '1', NODE_OPTIONS: nodeOptions },
  });
