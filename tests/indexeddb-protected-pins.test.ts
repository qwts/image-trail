...
test('IndexedDbBookmarkStore converts same-URL plaintext bookmarks before writing protected pins', async () => {
  await deleteImageTrailDb();
  ...
  try {
    const plain = await store.save(
      createDisplayRecord({
        id: 'https://secret.example.test/plain-then-protected.jpg',
        url: 'https://secret.example.test/plain-then-protected.jpg',
