...
- The attached Settings section is a layout-reserved scroll region (#367): while Settings is open it always occupies the same block size (`min(420px, 45vh)`), settings groups expand and collapse inside that region, and opening or closing a group never moves the clicked group control or any section rendered below Settings.
- The Settings region's internal scroll position survives the full-panel rerender caused by applying a setting (it participates in the render scroll snapshot alongside the parsed-field list and the queue list).
- A detached Settings window keeps its natural window-sized layout; the reserved-height rule applies only to the attached panel.
