...
## Failure Feedback Control (#450)

1. Open Settings → Automation → Preload.
2. Expected: a **Failure feedback** dropdown with Alert / Display / Mute, defaulting to Mute.
3. Change it; expected: the choice applies immediately (no separate Apply needed) and persists across a panel reopen.
4. See Acceptance-Test-Host-Image-Projection-Ownership for the per-mode visible-feedback behavior it drives.
