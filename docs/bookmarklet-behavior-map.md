...
The bookmarklet stored entries in append order. Current extension UI no longer exposes legacy migration controls; this note remains only as behavior-history context.
...
#### Extension shortcut decisions

- `p` is the extension's in-page hide/close shortcut. It closes the panel; reopening is handled by the toolbar action or the browser-level Image Trail shortcut command exposed in Chromium extension shortcuts.
- Legacy `h` hide/grayscale is intentionally not assigned (#391). It duplicates panel hiding and would add a separate selected-image grayscale style state outside the current target display controls.
- Legacy `a`–`z` field jumps are intentionally not assigned (#391). Letter keys already carry extension actions, and current navigation uses explicit field selection, per-field `+`/`-`, and Prev/Next/arrows over included navigable fields.
- The Settings Shortcuts group is the user-facing reference for in-page shortcuts that Chromium cannot register as extension commands.
...
| Automatic migration of existing localStorage data     | Not in scope                                   |
