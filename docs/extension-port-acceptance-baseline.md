...
- [ ] Extension code writes durable queue records to `bookmarks`; the old `favorites` naming remains historical context only.
...
- [x] Letter keys `a`–`z` are intentionally not assigned (#391); field selection is explicit and Prev/Next/arrows step included navigable fields together.
- [x] `h` hide/grayscale is intentionally not assigned (#391); `p` hides/closes the panel, and selected-image styling remains controlled by target display settings.
- [x] Browser-level panel open/hide is exposed through the Image Trail action command for Chromium extension shortcuts.
- [x] In-page shortcuts are listed in Settings so users can see available page-level keys.
...
| Legacy localStorage data              | Not supported in current UI |
...
| Automatic bookmarklet data migration  | Not in scope                |
