...
- Use `bookmarks` in new code; `favorites` remains historical bookmarklet terminology only.
...
As a user, I want to recover older encrypted records and move data between installs without exposing plaintext. This milestone adds recall/decrypt flows, encrypted import/export, password wrapping, key import/export, and encrypted download envelopes.
