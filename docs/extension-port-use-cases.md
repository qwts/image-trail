> Originally moved from `docs/extension-port-use-cases.md` during issue
> [#71](https://github.com/qwts/image-trail/issues/71).
...
- Orphan cleanup compares plaintext operational blob IDs against referenced blob IDs from durable bookmarks and browser-session recent-history rows.
- An unreferenced original is counted and selected for cleanup only after its creation time is at least one hour old. This grace period protects the capture-to-durable-link handoff; invalid or future creation times fail safe and remain protected.
- The unused-original count and cleanup action use the same eligible-ID set, so the displayed count matches what cleanup can hard-delete.
...
- Manual orphan tests are easiest when deleting bookmark metadata while leaving an older blob row intact; newly captured rows remain protected throughout the one-hour grace period.
