...
**Decision:** Accepted — adopt React incrementally for `ui/` rendering; retain the current esbuild extension bundle and Vite-based Storybook. See [ADR-0002](../adr/0002-react-ui-renderer-boundary.md) and [#539](https://github.com/qwts/image-trail/issues/539).
...
- Bundle growth and old DOM/CSS removal remain measured per migration slice.
- Replacing the packaged extension's esbuild pipeline with Vite is deferred; Vite already owns Storybook and interaction tests.
