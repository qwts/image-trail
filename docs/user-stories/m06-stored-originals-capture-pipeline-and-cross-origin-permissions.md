...
- Add an explicit repair action for selected durable pins whose stored-original reference is absent or points to a missing encrypted blob.
...
- Selected repair verifies encrypted blob references with key-only existence checks that do not load or serialize ciphertext, re-captures only missing originals through the existing origin-permission flow, and preserves durable queue order.
...
- Remove a selected pin's encrypted blob while retaining its metadata reference, run repair, and verify that the original is re-captured without changing `queueUpdatedAt`.
...
- A permission-needed capture names the required origin and offers **Grant permission and retry** beside **Dismiss**. Approval retries the same target/history/bookmark capture automatically; denial preserves a retryable failure.
- Missing-original repair is an explicit action on selected durable queue rows. It verifies referenced encrypted blobs before capture, reuses the existing permission-aware capture pipeline, pauses at the first permission boundary, and does not change `queueUpdatedAt`.
