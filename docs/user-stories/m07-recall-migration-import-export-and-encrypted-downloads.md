...
This milestone adds recall/decrypt flows, encrypted import/export, password wrapping, key import/export, and encrypted download envelopes.
...
- Keep retired legacy migration controls out of the current UI; use encrypted extension import/export for portability.
...
- Encrypted extension import/export works through explicit user actions and fails closed.
...
- **Most important build guardrails:** clean-profile restore, password wrapping, versioned file headers, partial-failure reporting.
- **Acceptance criteria added from review:** recall into runtime history and encrypted archive/key/download contracts.
...
- Use importer/exporter helpers per supported extension format and a shared validation layer.
...
- The story did not call out the concrete pass/fail acceptance criteria needed to prove recall into runtime history and encrypted archive/key/download contracts.
...
- Added a documentation-review completion block tying this story to: Brave extension port plan export/key-wrap assumptions, bookmarklet storage shape, encrypted download requirements.
- Added concrete acceptance scenarios for recall into runtime history and encrypted archive/key/download contracts.
- Added implementation notes that preserve clean-profile restore, password wrapping, versioned file headers, partial-failure reporting.
