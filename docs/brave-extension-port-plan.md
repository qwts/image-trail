...
- Preview styling can make the selected image the primary visible page item while preserving original DOM state for cleanup.
- When exactly one image is present on injection, auto-selection should happen immediately into the persisted preview layout, defaulting to fullscreen `contain`.
...
- Old plaintext localStorage migration is retired from the current UI; encrypted extension export/import is the supported portability path.
...
- Add encrypted extension export/import as the supported data-portability path.
...
- Immediate auto-selection when injection finds exactly one image, with fullscreen preview as the default persisted layout and an explicit page-layout toggle.
