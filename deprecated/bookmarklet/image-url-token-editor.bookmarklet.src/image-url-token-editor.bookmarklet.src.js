...
(function () {
    ...
    try {
      ...
      if (!state.panelSections || typeof state.panelSections !== 'object' || Array.isArray(state.panelSections)) {
        ...
        state.panelSections = Object.keys(PANEL_SECTION_DEFAULTS).reduce(function (acc, key) {
          acc[key] = typeof state.panelSections[key] === 'boolean' ? state.panelSections[key] : PANEL_SECTION_DEFAULTS[key];
          return acc;
        }, {});
      }
  ...
  function safeEncodeURIComponent(value) {
    return window.encodeURIComponent(String(value == null ? '' : value)).replace(/[!'()*]/g, function (char) {
  ...
  function findHistoryDownloadByUrl(normalizedUrl) {
    ...
    var entry = (app.settings.history || []).find(function (item) {
      return item && item.downloadedAt && normalizeAbsoluteUrl(item.url) === normalizedUrl;
    });
    return entry || null;
  }
  ...
  function ensureModelFilenameForUrl(url) {
    ...
    return runLlmMetadataFetch(url, 'title', { silent: true, strict: true }).then(function (value) {
      return ensureFilenameExtension(value || deriveTitle(url) || 'image', url);
    });
  }
  ...
  function getImageUrl(img) {
    ...
    return img.currentSrc || img.getAttribute('src') || img.src || img.getAttribute('data-src') || img.getAttribute('data-original') || '';
  }
  ...
  function collectFields(model) {
    ...
    model.pathParts.forEach(function (part, partIndex) {
      ...
      (part.tokens || []).forEach(function (token, tokenIndex) {
        ...
        var labelBase = isLikelyFilename(decodedSegment) ? 'file ' + tokenIndex : 'path ' + partIndex + '.' + tokenIndex;
  ...
  function applyFieldChangeToSelectedHistory(field) {
    ...
    app.selectedHistoryUrls = app.selectedHistoryUrls.map(function (url) {
    ...
    if (app.historySelectionAnchorUrl && urlMap[app.historySelectionAnchorUrl])
      app.historySelectionAnchorUrl = urlMap[app.historySelectionAnchorUrl];
  ...
  function ensureTargetIndicatorStyles() {
    ...
    app.targetIndicatorStyleEl = createEl('style', {
      text:
        'img[data-img-nav-host-selected="1"]{' +
  ...
  function clearTargetIndicator() {
    Array.prototype.slice.call(document.querySelectorAll('img[data-img-nav-host-selected="1"]')).forEach(function (img) {
  ...
  function addHistory(url) {
    if (!url) return;
    var existingEntry =
      (app.settings.history || []).find(function (item) {
    ...
    return filenamePromise
      .then(function (filename) {
        ...
        return computeImageFingerprint(url).then(function (fingerprint) {
          ...
          try {
            isCrossOrigin = new URL(normalizedUrl).origin !== location.origin;
          } catch (e) {}
          ...
          if (isCrossOrigin) {
            return downloadImageViaCanvas(url, filename).then(function (ok) {
              ...
              if (
                !(app.settings.history || []).some(function (entry) {
  ...
  function triggerPreloads() {
    var maxAttempts = app.settings.autoAdvanceOn404 ? Math.max(1, parseInt(app.settings.autoCount, 10) || 10) : 1;
  ...
  function startSlideshow() {
    ...
    app.autoRemaining = Number.isFinite(count) && count > 0 ? count : Number.POSITIVE_INFINITY;
      ...
      section(
        ...
        [
          ...
          createEl(
            ...
            [
              button('Apply URL', function () {
                parseAndApplyUrl(app.fullUrlEl.value);
              }),
              button('Save State', function () {
                saveState();
  ...
  function renderControls() {
    ...
    return [
      ...
      direction,
      label('Step'),
      input(app.settings.step, function (value) {
        app.settings.step = value;
        saveState();
      }),
      label('Slideshow count (0 = infinite)'),
      input(app.settings.autoCount, function (value) {
        app.settings.autoCount = value;
        saveState();
      }),
      label('Slideshow pause ms'),
      input(app.settings.slideshowPause, function (value) {
        app.settings.slideshowPause = value;
        saveState();
      }),
      label('404 retry delay ms'),
      input(app.settings.autoDelay, function (value) {
        app.settings.autoDelay = value;
        saveState();
      }),
      createEl(
        'label',
      ...
      input(app.settings.auto404Count, function (value) {
        app.settings.auto404Count = value;
        saveState();
      }),
      label('LLM endpoint'),
  ...
  function setHistorySelectionByIndex(index, options) {
    ...
    if (options.range) {
      ...
      nextSelection = entries.slice(start, end + 1).map(function (entry) {
        return entry.url;
      });
  ...
  function loadSelectedHistoryItem() {
    ...
    var url =
      app.historyFocusedUrl && app.selectedHistoryUrls.indexOf(app.historyFocusedUrl) >= 0
        ? app.historyFocusedUrl
  ...
  function renderHistory() {
    ...
    app.historyEl.appendChild(
      createEl('div', { style: Object.assign({}, gridStyle, { gridTemplateColumns: 'repeat(2, 1fr)' }) }, [
        button(
          'Clear Downloads',
          function () {
    ...
    app.historyEl.appendChild(
      createEl('div', { style: { marginBottom: '6px' } }, [
        button(
          'Clear Storage',
          function () {
    ...
    getVisibleHistoryEntries().forEach(function (item, historyIndex) {
      ...
      var row = createEl('div', {
        style: {
          display: 'grid',
          gridTemplateColumns: showThumbnails ? HISTORY_THUMBNAIL_SIZE + 6 + 'px 1fr auto' : '1fr auto',
          gap: '5px',
          alignItems: 'center',
  ...
  function findImageFromEvent(event) {
    ...
    if (typeof document.elementsFromPoint === 'function' && Number.isFinite(event.clientX) && Number.isFinite(event.clientY)) {
