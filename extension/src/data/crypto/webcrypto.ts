...
export async function encryptAesGcm(
  ...
  const result = await getCrypto().subtle.encrypt(params, key, plaintext as BufferSource);
...
export async function decryptAesGcm(
  ...
  const params: AesGcmParams = { name: 'AES-GCM', iv: iv as BufferSource };
  if (additionalData) params.additionalData = additionalData as BufferSource;
  const result = await getCrypto().subtle.decrypt(params, key, ciphertext as BufferSource);
  return new Uint8Array(result);
}
