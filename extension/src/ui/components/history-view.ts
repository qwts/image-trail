...
export function createHistoryView(
  ...
  dispatch: (action: { readonly name: 'history/remove' | 'capture/history'; readonly id: string }) => void,
): HTMLElement {
  ...
  for (const item of items) {
    ...
    capture.addEventListener('click', () => dispatch({ name: 'capture/history', id: item.id }));
    const remove = document.createElement('button');
    remove.type = 'button';
    remove.textContent = 'Remove';
    remove.addEventListener('click', () => dispatch({ name: 'history/remove', id: item.id }));
    entry.append(link, capture, remove);
    list.append(entry);
  }
