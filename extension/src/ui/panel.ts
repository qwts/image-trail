...
import type { BookmarkStore, CaptureStore, PanelAction, PanelState, TargetState } from '../core/types.js';
...
export class ImageTrailPanel {
  ...
  private dispatch = (action: PanelAction): void => {
    ...
    if (action.name === 'capture/current') {
      void this.captureUrl(this.state.target.selectedUrl);
      return;
    }

    if (action.name === 'capture/history') {
      void this.captureUrl(this.state.history.find((item) => item.id === action.id)?.url ?? null);
      return;
    }

    if (action.name === 'capture/bookmark') {
      void this.captureUrl(this.state.bookmarks.find((item) => item.id === action.id)?.url ?? null);
      return;
    }
  ...
  private async captureUrl(url: string | null): Promise<void> {
    if (!url || !this.captureStore) return;
    const result = await this.captureStore.capture(url);
    this.state = reducePanelAction(this.state, { name: 'capture/result', result });
    this.render();
  }
