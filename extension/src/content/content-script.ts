...
import { BackgroundCaptureStore } from './capture-controller.js';
...
function createController(): ImageTrailContentController {
  ...
  const panel = new ImageTrailPanel(pageAdapter, new IndexedDbBookmarkStore(), new BackgroundCaptureStore());
  ...
  const handleMessage = (message: unknown, _sender: chrome.runtime.MessageSender, sendResponse: (response: unknown) => void): boolean => {
    ...
    switch (message.type) {
      ...
      case MessageType.Ping:
        ...
        return false;
      case MessageType.CaptureOriginal:
        sendResponse(createUnknownMessageResponse('Capture requests are handled by the background service worker.'));
        return false;
    }
  };
