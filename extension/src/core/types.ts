...
import type { CaptureResult, StorageUsageSnapshot } from './image/image-metadata.js';
...
export interface PanelState {
  ...
  readonly storageUsage: StorageUsageSnapshot | null;
}
...
export type PanelAction =
  ...
  | { readonly name: 'history/remove' | 'bookmark/load' | 'bookmark/remove' | 'capture/history' | 'capture/bookmark'; readonly id: string }
  | { readonly name: 'capture/result'; readonly result: CaptureResult };
...
export interface CaptureStore {
  readonly capture: (url: string) => Promise<CaptureResult>;
}
